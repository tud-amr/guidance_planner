...
#pragma omp parallel for num_threads(8)
    ...
    void Environment::ObstacleGrid::SetOrigin(const Eigen::Vector2d &pos)
    {
    ...
    void Environment::ObstacleGrid::Clear()
    {
#pragma omp parallel for num_threads(8)
        for (size_t k = 0; k < grid_.size(); k++)
        {
    ...
    void Environment::ObstacleGrid::InsertObstacle(int k, const SingleObstacle &obstacle)
    {
        int i_x, i_y;
...
#pragma omp parallel for
    ...
    std::vector<Environment::SingleObstacle> &Environment::ObstacleGrid::ObstaclesAt(const SpaceTimePoint &point)
    {
        if (grid_.size() == 0)
    ...
    void Environment::ObstacleGrid::Print()
    {
        std::cout << "Grid: [" << grid_.size() << " x " << grid_[0].size() << " x " << grid_[0][0].size() << "]" << std::endl;
    }

    bool Environment::ObstacleGrid::OutOfGridBounds(int i_x, int i_y)
    {
        return i_x < 0 || i_x >= size_x_ || i_y < 0 || i_y >= size_y_; // If it is outside the grid
    }

    void Environment::ObstacleGrid::GetGridIndices(const Eigen::Vector2d &pos, int &i_x_out, int &i_y_out)
    {
        // Find the grid index
    ...
    bool Environment::InCollision(const SpaceTimePoint &point, double with_margin)
    {
        ...
        // std::cout << "number of obstacles there: " << grid_[point.Time()][i_x][i_y].size() << std::endl;

        for (auto &obstacle : cur_grid_obstacles)
        {
            if (Helpers::dist(obstacle.position, point.Pos()) < obstacle.radius + with_margin) // Note that the obstacle positions at k = 0 is the initial state
                return true;
        }
    ...
    void Environment::LoadObstacles(const std::vector<Obstacle> &dynamic_obstacles, const std::vector<Halfspace> &static_obstacles)
    {
        ...
        grid_.SetDimensions(30, 30, dynamic_obstacles_[0].radius_);
        grid_.Clear();
    ...
    bool Environment::IsVisible(const Node &a, const Node &b)
    {
        return IsVisible(a.point_, b.point_);
    }

    bool Environment::IsVisible(const SpaceTimePoint &point_one, const SpaceTimePoint &point_two)
    {
        return IsVisibleRayCast(point_one, point_two);
    }
