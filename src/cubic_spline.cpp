...
void CubicSpline3D::ConvertToTrajectory(const GeometricPath &path)
{
    ...
    // control_points_ = ControlPoints(path.nodes_.size() - 2); // config_->num_points_ - 2); // First point is the start / last the end // HomotopyConfig::N - 1);
...
void CubicSpline3D::Optimize(const std::vector<Obstacle> &obstacles)
{
    ...
    for (int repeat_id = 0; repeat_id < config_->repeat_times_; repeat_id++)
    {
        ...
        Eigen::VectorXd f_g = Eigen::Map<Eigen::VectorXd>(initial_control_points_.points_.data(), initial_control_points_.points_.cols() * initial_control_points_.points_.rows());

        // Simply identity weights
        Eigen::MatrixXd H_g = config_->geometric_weight_ * 2. * Eigen::MatrixXd::Identity(control_points_.NumPoints() * 2, control_points_.NumPoints() * 2);
        ...
        for (int i = 0; i < control_points_.NumPoints(); i++)
        {
            ...
            {
                // Index 1 - N
                Eigen::Vector2d obstacle_pos = obstacle.positions_[std::round(control_points_.GetTime(i) / HomotopyConfig::DT)]; // std::round(control_points_(2, i))];
        ...
        Eigen::VectorXd f_v = Eigen::Map<Eigen::VectorXd>(velocity_spline_control_points_.data(), velocity_spline_control_points_.cols() * velocity_spline_control_points_.rows());

        // Simply identity weights
        Eigen::MatrixXd H_v = config_->velocity_tracking_ * 2. * Eigen::MatrixXd::Identity(control_points_.NumPoints() * 2, control_points_.NumPoints() * 2);
...
Eigen::MatrixXd CubicSpline3D::ComputeVelocitySplinePoints()
{
    ...
    for (int i = 0; i < velocity_control_points_.cols(); i++)
        velocity_control_points_.col(i) = velocity_control_points_.col(i).normalized().array() * HomotopyConfig::reference_velocity_; /** @todo input vref */
    ...
    if (config_->smoothen_velocity_)
    {
        ...
        start << previous_velocity_(0), previous_velocity_(1), previous_velocity_(0), previous_velocity_(1); // velocity_control_points_(0, 0), velocity_control_points_(1, 0);

        // Last velocity, twice
        end = Eigen::VectorXd(4);
        end << velocity_control_points_.rightCols(1)(0), velocity_control_points_.rightCols(1)(1), velocity_control_points_.rightCols(1)(0), velocity_control_points_.rightCols(1)(1);
...
CubicSpline2D<tk::spline> &CubicSpline3D::GetPath()
{
    return *path_spline_;
}

CubicSpline2D<tk::spline> &CubicSpline3D::GetTrajectory() const
{
    return *trajectory_spline_;
}
...
double CubicSpline3D::WeightPathLength()
{
    return path_spline_->GetSplineLength();
}
...
double CubicSpline3D::WeightVelocity()
{
    ...
    for (int i = 0; i < t_sampled.rows(); i++)
    {
        ...
        result += (HomotopyConfig::reference_velocity_ - cur_velocity) * (HomotopyConfig::reference_velocity_ - cur_velocity); // Quadratic error w.r.t. the reference
    }
...
void CubicSpline3D::Visualize(ROSMarkerPublisher *ros_visuals)
{
    ...
    for (int i = 0; i < control_points_.NumPoints(); i++)
    {
        cube.addPointMarker(Eigen::Vector3d(
            control_points_.GetPoint(i)(0),
            control_points_.GetPoint(i)(1),
            control_points_.GetTime(i)));
    }
    ...
    {
        cube.addPointMarker(Eigen::Vector3d(
            initial_control_points_.GetPoint(i)(0),
            initial_control_points_.GetPoint(i)(1),
            initial_control_points_.GetTime(i)));
    }
    ...
    for (int i = 0; i < 1; i++)
    {
        ...
        cube.setScale(0.15, 0.15, 0.15);
        line.addLine(Eigen::Vector3d(
                         control_points_.GetPaddedPoint(i)(0),
                         control_points_.GetPaddedPoint(i)(1),
                         control_points_.GetPaddedTime(i)),
                     Eigen::Vector3d(control_points_.GetPaddedPoint(i)(0) + current_velocity_(0),
                                     control_points_.GetPaddedPoint(i)(1) + current_velocity_(1),
                                     control_points_.GetPaddedTime(i)));
        // }
        cube.addPointMarker(Eigen::Vector3d(
            control_points_.GetPaddedPoint(i)(0),
