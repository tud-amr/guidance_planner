...
namespace GuidancePlanner
{
    ...
    struct SpaceTimePoint
    {
        ...
        SpaceTimePoint(const Eigen::Vector3d &other)
        {
            vec = other;
        }

        SpaceTimePoint(const SpaceTimePoint &other)
        {
            *this = other;
        }

        SpaceTimePoint(double x, double y, double k)
        {
            vec = Eigen::Vector3d(x, y, k);
        }
        ...
        {
            return Eigen::Vector3d(vec(0), vec(1), vec(2) * HomotopyConfig::DT);
        }
        ...
        SpaceTimePoint operator+(const SpaceTimePoint &other) const
        {
            return SpaceTimePoint(vec + other.vec);
        }
        ...
        SpaceTimePoint operator*(const double &other) const
        {
            return SpaceTimePoint(other * vec);
        }

        friend SpaceTimePoint operator*(const double &first, const SpaceTimePoint &other)
        {
            return SpaceTimePoint(first * other.vec);
        }
    };
    ...
    struct Node
    {
        ...
        int belongs_to_path_; /** @note Set a posteriori for visualization */
        ...
        void SetSegmentAssociation(const int segment_id)
        {
            segment_association_id_ = segment_id;
        }

        double DistanceTo(const Node &other)
        {
            return ((Eigen::Vector3d)(other.point_ - this->point_)).norm();
        }

        double DistanceTo(const Node &other, double dt)
        {
            return (other.point_.MapToTime() - this->point_.MapToTime()).norm();
        }
    ...
    public:
        ...
        /** @brief Initialize the graph with a start and goal */
        void Initialize(const Eigen::Vector2d &start, const std::vector<Eigen::Vector2d> &goal_points) // To PRM?
        {
            nodes_.emplace_back(-1, SpaceTimePoint(start(0), start(1), 0), NodeType::GUARD); // Add start
            start_node_ = &nodes_.back();

            goal_nodes_.clear();
            for (auto &point : goal_points) // Create multiple goals
            {
                nodes_.emplace_back(-(int)goal_nodes_.size() - 2, SpaceTimePoint(point(0), point(1), HomotopyConfig::N), NodeType::GOAL); // Add goal
                goal_nodes_.push_back(&nodes_.back());
            }
        }
        ...
        std::vector<Node *> GetSharedNeighbours(const std::vector<Node *> &nodes)
        {
            ...
            for (auto &neighbour : nodes[0]->neighbours_) // For neighbours of the first node
            {
                ...
                {
                    if (neighbour->id_ == other_neighbour->id_ || (neighbour->type_ == NodeType::GOAL && other_neighbour->type_ == NodeType::GOAL)) // If neighbours are the same, add them to the shared neighbours
