...
namespace GuidancePlanner
{
    ...
    // {
    //     typedef Eigen::Matrix<double, T, 1> Vector;
    ...
    //     {
    //         static_assert(sizeof...(args) == T - 1, "Number of arguments must be equal to T");
    ...
    //     operator Eigen::Matrix<double, T, 1>() const { return _vec; } // Cast to Vector

    //     Eigen::Vector2d Pos() const { return _vec.block<T - 1, 1>(0, 0); }
    //     void SetPos(const Eigen::Matrix<double, T - 1, 1> &val) { _vec.block<T - 1, 1>(0, 0) = val; }

    //     double &Time() { return _vec(T - 1); }
    //     double Time() const { return _vec(T - 1); }
    ...
    // typedef SpaceTimePointDim<3> SpaceTimePoint;

    // Wrapper for vector3d
    /*struct SpaceTimePointDeprecated
    {
        Eigen::Vector3d vec;
